...
export default function HeaderNav() {
    ...
    return (
        <Navbar isBordered onMenuOpenChange={setExpanded_nav} isMenuOpen={expanded_nav} classNames={{
            item: [
            "flex",
            "relative",
            "h-full",
        ...
        >
            <NavbarMenuToggle
                ...
                className="sm:hidden"
            />
            <NavbarBrand as={RouterLink} to="/" onClick={()=>setExpanded_nav(false)} className="flex items-center">
                <Image src={VinylLogo} alt="VinylByte Logo" className="w-10 h-10 mr-2" />
                <p className="font-bold text-inherit text-xl">HitLab</p>
            </NavbarBrand>
            <NavbarMenu>
                {Links.map(item => (
                <NavbarMenuItem key={`${item.name}-${item.to}`} isActive={currentHref === item.to} className="pt-2 pb-2">
                    <Link 
                        as={RouterLink}
                        className="w-full"
                        to={item.to}
            ...
            <NavbarContent className="hidden sm:flex gap-4" justify="center">
                {
                    Links.map((link) => (
                        <NavbarItem isActive={currentHref === link.to} key={link.to}>
                            <Link as={RouterLink} to={link.to} color={currentHref === link.to ? "primary" : "foreground"} className="w-30 h-15">
                                <Center w={"100%"}>
